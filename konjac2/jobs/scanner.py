...
from ..strategy.bb_adx_rsi_strategy_v3 import BBAdxRsiV3
...
async def smart_dog(currency="DOGE"):
    ...
    spot_symbol = f"{currency}/USD"
    future_symbol = f"{currency}/USDT"
    strategy = BBAdxRsiV3(symbol=spot_symbol)
    # somehow gemini only return finished timeframe data
...
async def scanner_job():
    ...
    for instrument in Instruments:
        await trade_forex(symbol=instrument, timeframe="M5", trading_strategy=BBAdxRsiV3, quantity=5000)
    # await smart_dog()
    # await smart_dog("ADA")
