...
class BBAdxRsi(ABCStrategy):
    strategy_name = "bb adx rsi"
    ...
    def seek_trend(self, candles, day_candles=None):
        ...
        if trend[-1] > 0 and adx_value < 30 and rsi_data[-2] <= 30 and rsi_data[-1] > 30:
            self._delete_last_in_progress_trade()
            self._start_new_trade(TradeType.long.name, candles.index[-1], h4_date=day_candles.index[-1])
        if trend[-1] < 0 and adx_value < 30 and rsi_data[-2] >= 70 and rsi_data[-1] < 70:
            self._delete_last_in_progress_trade()
