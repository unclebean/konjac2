...
class BBHmaRsi(ABCStrategy):
    strategy_name = "bb hma rsi"

    def __init__(self, symbol: str, trade_short_order=True):
        ABCStrategy.__init__(self, symbol, trade_short_order)

    def seek_trend(self, candles, day_candles=None):
        hma_data = hma(candles.close, length=15)
        rsi_data = rsi(candles.close, length=5)
        bb_20 = bbands(candles.close, length=20, std=2.5)
        bb_20_low = bb_20["BBL_20_2.5"]
        bb_20_up = bb_20["BBU_20_2.5"]
        if rsi_data[-1] <= 30 \
                and hma_data[-1] > candles.close[-1] \
                and (bb_20_low[-1] > candles.close[-1]
                     or (candles.open[-1] < bb_20_low[-1] < candles.close[-1])):
        ...
        if rsi_data[-1] >= 70 \
                ...
                and (bb_20_up[-1] < candles.close[-1]
                     or (candles.open[-1] > bb_20_up[-1] > candles.close[-1])):
            self._delete_last_in_progress_trade()
    ...
    def exit_signal(self, candles, day_candles=None) -> bool:
        ...
        if last_order_status.ready_to_procceed \
                and last_order_status.is_long \
                and (is_profit or is_loss or hma_data[-1] > candles.close[-1]):
